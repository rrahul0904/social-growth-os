...
export async function enqueueJob(type: JobType, payload: Record<string, unknown>) {
  if (isDemoMode()) return { id: `demo_job_${crypto.randomUUID()}`, type, payload };
  const sql = db();
  const [row] = await sql`
...
export async function leaseNextJob(workerId: string, leaseSeconds = 90): Promise<QueueJob | null> {
...
export async function completeJob(jobId: string) {
  ...
  const sql = db();
  await sql`update job_queue set status='completed', completed_at=now(), updated_at=now() where id=${jobId}`;
}
...
export async function failJob(jobId: string, error: string) {
  ...
  await sql`
    ...
    set status = case when attempts >= max_attempts then 'dead' else 'queued' end,
        ...
        run_at = now() + (least(300, power(2, attempts) * 5)::int || ' seconds')::interval,
        updated_at = now()
    where id = ${jobId}
  `;
}
