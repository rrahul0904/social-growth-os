...
export async function enqueueJob(type:JobType,payload:Record<string,unknown>,workspaceId:string){if(isDemoMode())return{id:`demo_job_${crypto.randomUUID()}`,workspaceId,type,payload};const sql=db();const [row]=await sql`insert into job_queue (workspace_id,type,payload) values (${workspaceId}::uuid,${type},${sql.json(payload as any)}) returning id,workspace_id::text as "workspaceId",type,payload`;return row}
